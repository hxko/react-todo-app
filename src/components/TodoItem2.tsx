...
const TodoItem: React.FC<TodoItemTypes> = ({ completed, id, title }) => {
  ...
  const [editedTitle, setEditedTitle] = useState("");   // Handle EDITING title

  const handleEditStart = () => {
    setIsEditing(true);
  };

  // focus title when isEditing 
  useEffect(() => {
    if (titleRef.current) {
  ...
  const handleCancelEdit = () => {
    console.log("cancel", title, editedTitle);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {editedTitle ? editedTitle : (editedTitle == '' ? 'Enter Title' : title)}
        {isEditing &&
